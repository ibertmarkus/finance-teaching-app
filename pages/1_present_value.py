...
# Title
st.title("Present Value: PV = FV / (1+r)^t")
st.write("If I want FV dollars in t years at an interest rate r, how much do I need to invest today?")
...
with left_col:
    ...
    future_value_1 = st.number_input("Future Value Desired ($)", min_value=0.0, max_value=1000000.0, value=100.0, step=10.0, format="%.2f", key="fv1")
    ...
    present_value_1 = future_value_1 / (1 + r1) ** time_period_1
    st.metric("Present Value Needed", f"${present_value_1:,.2f}")

    # Scenario 2
    st.markdown("---")
    st.markdown("**Scenario 2 (Optional)**")
    future_value_2 = st.number_input("Future Value Desired ($)", min_value=0.0, max_value=1000000.0, value=0.0, step=10.0, format="%.2f", key="fv2")
    ...
    if future_value_2 > 0:
        ...
        present_value_2 = future_value_2 / (1 + r2) ** time_period_2
        st.metric("Present Value Needed", f"${present_value_2:,.2f}")

    # Scenario 3
    st.markdown("---")
    st.markdown("**Scenario 3 (Optional)**")
    future_value_3 = st.number_input("Future Value Desired ($)", min_value=0.0, max_value=1000000.0, value=0.0, step=10.0, format="%.2f", key="fv3")
    ...
    if future_value_3 > 0:
        ...
        present_value_3 = future_value_3 / (1 + r3) ** time_period_3
        st.metric("Present Value Needed", f"${present_value_3:,.2f}")

with right_col:
    # Determine max time period for x-axis
    max_time = max(time_period_1, time_period_2 if future_value_2 > 0 else 0, time_period_3 if future_value_3 > 0 else 0)
    ...
    present_values_1 = future_value_1 / (1 + r1) ** years_1
    ax.plot(years_1, present_values_1, linewidth=1.5, color=colors[0], marker='o', markersize=3,
            label=f'Scenario 1: ${future_value_1:,.0f} @ {interest_rate_1}%')

    # Plot Scenario 2 if active
    if future_value_2 > 0:
        years_2 = np.arange(0, time_period_2 + 1)
        present_values_2 = future_value_2 / (1 + r2) ** years_2
        ax.plot(years_2, present_values_2, linewidth=1.5, color=colors[1], marker='o', markersize=3,
                label=f'Scenario 2: ${future_value_2:,.0f} @ {interest_rate_2}%')

    # Plot Scenario 3 if active
    if future_value_3 > 0:
        years_3 = np.arange(0, time_period_3 + 1)
        present_values_3 = future_value_3 / (1 + r3) ** years_3
        ax.plot(years_3, present_values_3, linewidth=1.5, color=colors[2], marker='o', markersize=3,
                label=f'Scenario 3: ${future_value_3:,.0f} @ {interest_rate_3}%')

    # Formatting
    ax.set_xlabel('Time (years)', fontsize=9, fontweight='bold')
    ax.set_ylabel('Present Value ($)', fontsize=9, fontweight='bold')
    ax.set_title('Present Value Comparison', fontsize=10, fontweight='bold')
...
# Optional: Show the data table - Growth from PV to FV (Scenario 1 only)
with st.expander("View Year-by-Year Growth from Present Value to Future Value (Scenario 1)"):
    import pandas as pd

    # Calculate how the present value grows to the future value
    growth_values = present_value_1 * (1 + r1) ** years_1

    # Calculate components for each year
    simple_interest_principal = present_value_1 * r1
    growth_from_prev = [0] + [growth_values[i] - growth_values[i-1] for i in range(1, len(years_1))]
    ...
    df = pd.DataFrame({
        'Year': years_1,
        'Value': [f'${fv:,.2f}' for fv in growth_values],
